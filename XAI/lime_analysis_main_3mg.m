%% --- Archivo: lime_analysis_main_3mg.m ---
%
% Script Principal: Análisis LIME Cooperativo (Sistema 2).
% ** ACTUALIZACIÓN: Bucle para explicar Q_t de MG1, MG2 y MG3 secuencialmente.
%
%--------------------------------------------------------------------------
close all; clear; clc;

K_TARGET = 8641; % Instante de análisis
NUM_RUNS = 3;   % Ejecuciones para estabilidad

fprintf('--- LIME COOPERATIVO 3-MG (Análisis Completo) ---\n');

% 1. Reconstruir Estado
fprintf('1. Reconstruyendo estado del sistema...\n');
[estado, params] = reconstruct_state_matlab_3mg(K_TARGET);

% 2. Compilar Controlador
fprintf('2. Compilando optimizador YALMIP 3-MG...\n');
controller_obj = get_compiled_mpc_controller_3mg(params.mg);

% 3. BUCLE DE ANÁLISIS PARA LAS 3 MGs
targets = [1, 2, 3]; 

for t_idx = targets
    fprintf('\n============================================\n');
    fprintf('   INICIANDO ANÁLISIS PARA AGENTE: MG %d\n', t_idx);
    fprintf('============================================\n');
    
    val_real = estado.Y_target_real_vector(t_idx);
    fprintf('Valor Real Q_t(MG%d) en K=%d: %.4f L/s\n', t_idx, K_TARGET, val_real);
    
    % Ejecutar LIME pasando el índice del target
    [all_explanations, feature_names] = calculate_lime_stability_3mg( ...
        estado, controller_obj, params, NUM_RUNS, t_idx);
    
    % Guardar Resultados Específicos
    filename = sprintf('lime_3mg_k%d_MG%d.mat', K_TARGET, t_idx);
    
    % Guardamos t_idx para referencia en el plotter
    target_mg_idx = t_idx; 
    save(filename, 'all_explanations', 'feature_names', 'estado', 'K_TARGET', 'target_mg_idx');
    
    fprintf('Resultados para MG%d guardados en: %s\n', t_idx, filename);
end

fprintf('\n--- ANÁLISIS COMPLETO FINALIZADO ---\n');

%% --- Helper: Función para Compilar el MPC 3-MG ---
function Controller = get_compiled_mpc_controller_3mg(mg_array)
    ...
    for k = 1:N
        % Pozo Simplificado para Optimizer
        for i = 1:num_mg
            descenso = 0;
    ...
    fprintf('Controlador 3-MG compilado.\n');
end