%% --- Archivo: RUN_LIME_ALL_MASTER.m ---
%
% EL SCRIPT DEFINITIVO: EJECUCIÓN MASIVA LIME PARA TESIS
%
% Ejecuta y guarda todas las combinaciones de la Matriz Experimental:
% - Modelos: AR, TS
% - Casos: 1 (Mean), 2 (Standard Moments), 3 (AE Moments)
% - Perturbaciones: GAUSSIAN, PARETO
% - Targets: Intercambio (Q_t) [Todos los escenarios] y Bombeo (Q_p) [Altruismo]
%
% FUNCIONALIDAD EXTRA: Escanea recursivamente toda la carpeta del proyecto
% para encontrar cualquier función (submuestreo_max, calculate_lime_pumping, etc.)
% FIX: Corrección lógica para forzar la evaluación de bombeo en TS.
%--------------------------------------------------------------------------
close all; clear; clc;

% --- MAGIA DE RUTAS (Path) ---
% Sube un nivel desde "XAI" para llegar a "sistema_micro-redes"
% y añade ABSOLUTAMENTE TODAS las subcarpetas a la ruta de búsqueda.
project_root = fullfile(pwd, '..'); 
addpath(genpath(project_root));
fprintf('--- RUTAS AÑADIDAS: MATLAB ahora ve todas las subcarpetas del proyecto ---\n');

% --- CONFIGURACIÓN GLOBAL ---
NUM_RUNS = 5;           % 10 Corridas para significancia estadística real
...
fprintf('=========================================================\n');
fprintf(' INICIANDO EJECUCIÓN MAESTRA LIME (%d Combinaciones)\n', 2*3*2*3*3);
fprintf('=========================================================\n\n');
...
for m_idx = 1:length(MODELS)
    ...
    mg = results.mg; Q_t = results.Q_t; Q_p = results.Q_p; Q_buy = results.Q_DNO; Q_dem_full = profiles.Q_dem_sim;
    ...
    k_alt_vec = zeros(1, 3);
    valid_altruism = false(1, 3); % BANDERA AÑADIDA: Para no discriminar al TS en bombeo
    for i=1:3
    ...
    for i=1:3
        ...
        if any(mask)
            ...
            scores(mask) = similarity .* purity .* import_val; [~, idx_sc] = max(scores); k_direct_vec(i) = indices_mpc(idx_sc);
        else, [~, idx_fb] = min(q_t_sub); k_direct_vec(i) = indices_mpc(idx_fb); end
    end
    ...
    for s_idx = 1:length(scenarios)
        ...
        for t_idx = 1:3
            ...
            fprintf('\n  [+] Evaluando MG%d en Escenario: %s (K=%d)\n', t_idx, scn.name, k_target);
            
            % Reconstrucción Pesada (Una vez por objetivo)
            [estado_raw, params_raw] = reconstruct_state_matlab_3mg(k_target, TIPO_MODELO);
            
            % Extracción de matrices completas para promedios/momentos
            ...
            for p_idx = 1:length(PERTURBATIONS)
                ...
                for c_idx = CASES
                    ...
                    % 2. EJECUCIÓN INTERCAMBIO (Q_t) - SIEMPRE
                    try
                        if c_idx == 1 % Mean
                            [lime_ex, expl_ex] = calculate_lime_stability_3mg_with_quality(estado, controller_obj, params_raw, NUM_RUNS, t_idx, PERTURBATION_TYPE);
                        elseif c_idx == 2 % Standard Moments
                            if strcmp(PERTURBATION_TYPE,'PARETO'), [lime_ex, expl_ex] = calculate_lime_exchange_3mg_STANDARD_MOMENTS_PARETO(estado, controller_obj, params_raw, NUM_RUNS, t_idx);
                            else, [lime_ex, expl_ex] = calculate_lime_exchange_3mg_STANDARD_MOMENTS(estado, controller_obj, params_raw, NUM_RUNS, t_idx); end
                        elseif c_idx == 3 % AE Moments
                            if strcmp(PERTURBATION_TYPE,'PARETO'), [lime_ex, expl_ex] = calculate_lime_exchange_3mg_with_AE_MOMENTS_PARETO(estado, controller_obj, params_raw, NUM_RUNS, t_idx);
                            else, [lime_ex, expl_ex] = calculate_lime_exchange_3mg_with_AE_MOMENTS(estado, controller_obj, params_raw, NUM_RUNS, t_idx); end
                        end
                        rbo_ex = compute_rbo_stats(expl_ex, NUM_RUNS, RBO_P);
                        
                        f_ex = fullfile(dirs_ex{c_idx}, sprintf('lime_Scenario_%s_%s_MG%d_%s_%s.mat', scn.name, TIPO_MODELO, t_idx, suf_ex{c_idx}, PERTURBATION_TYPE));
                        feature_names = estado.feature_names; lime_stats = lime_ex; rbo_stats = rbo_ex; all_explanations = expl_ex; K_TARGET = k_target; target_mg_idx = t_idx;
                        save(f_ex, 'all_explanations', 'feature_names', 'lime_stats', 'rbo_stats', 'K_TARGET', 'target_mg_idx', 'scn', 'estado');
                        fprintf('Q_t[R2:%.2f|RBO:%.2f] ', lime_ex.R2_mean, rbo_ex.mean);
                    catch ME
                        fprintf('Q_t[ERROR: %s] ', ME.message);
                    end
                    
                    % 3. EJECUCIÓN BOMBEO (Q_p) - SOLO SI ES ALTRUISMO VÁLIDO
                    % FIX APLICADO: Ahora usa la bandera 'valid_altruism'
                    if strcmp(scn.name, 'Altruismo') && valid_altruism(t_idx)
                        try
                            if c_idx == 1 % Mean
                        ...
                        fprintf('\n'); % Saltar línea si no hay bombeo que analizar
                    end
                end % Fin Casos
...
fprintf('\n=== TODO EL ANÁLISIS HA CONCLUIDO EXITOSAMENTE ===\n');
