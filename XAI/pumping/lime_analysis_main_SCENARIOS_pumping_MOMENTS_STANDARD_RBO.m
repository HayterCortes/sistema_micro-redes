%% --- Archivo: lime_analysis_main_SCENARIOS_pumping_MOMENTS_STANDARD_RBO.m ---
%
% SCRIPT MAESTRO DE ANÁLISIS DE ESCENARIOS BOMBEO (Q_p) - STANDARD (34 VARS)
%
% Objetivo: Explicar bombeo en escenarios de Altruismo sin usar Autoencoder (Benchmark).
% Features: 34 Variables (Estados + Momentos Estadísticos).
...
for i = 1:3
    ...
    if any(mask)
        % Lógica Ratio: Eficiencia de bombeo para exportación
        ...
        fprintf('  MG%d: K=%d (Score=%.3f) -> Candidato.\n', i, k_alt_vec(i), max(scores));
    else
...
for s_idx = 1:length(scenarios)
    ...
    for t_idx = 1:3
        ...
        % Si el target es el default, saltamos
        ...
        % --- C. EJECUTAR LIME STANDARD (Con Selector) ---
        ...
        filename = sprintf('lime_pumping_Scenario_%s_%s_MG%d_STANDARD_RBO_%s.mat', ...
