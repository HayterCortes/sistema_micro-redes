...
%
% CORRECCIÓN: Se actualiza explícitamente estado.Y_target_real_vector con Q_p
% para evitar que el plotter muestre valores de Q_t por error.
%
%--------------------------------------------------------------------------
close all; clear; clc;
...
for s_idx = 1:length(scenarios)
    ...
    for t_idx = 1:3
        ...
        % --- CORRECCIÓN CRÍTICA ---
        % Sobrescribimos el vector 'Y' real en el estado para que sea Q_p.
        % La función reconstruct_... original ponía Q_t aquí.
        estado.Y_target_real_vector = results.Q_p(k_target_actual, :); 
        % ---------------------------
        
        % 3. Valor real para referencia visual en consola
