%% --- Archivo: lime_temporal_pumping_main_MOMENTS_DENOISING.m ---
%
% LIME DENOISING + MOMENTS FULL (34 Vars)
%
% Combina la robustez de Denoising con la precisión de los Momentos (Max/Std)
% para TODAS las variables predictoras (P_gen, P_dem, Q_dem).
%
% CORRECCIÓN: Feature Engineering extendido a 34 variables y gestión de nombres.
%--------------------------------------------------------------------------
close all; clear; clc;

% --- 1. CONFIGURACIÓN ---
TIPO_MODELO = 'TS';      % <--- CAMBIA A 'AR' o 'TS'
TARGETS = [1, 2, 3];     
INTERVALO_HORAS = 12;    
NUM_RUNS_PER_POINT = 1;  

fprintf('--- LIME BOMBEO (MOMENTS FULL + DENOISING) - Modelo: %s ---\n', TIPO_MODELO);
...
try
    ...
    results = load(fname); mg = results.mg;
...
for k = k_list_raw
    ...
    if k_adjusted < Total_Steps, k_list = [k_list, k_adjusted]; end
end

% --- 4. BUCLE POR AGENTE ---
for t_idx = TARGETS
    fprintf('\n>>> MG %d (MOMENTS DENOISING) <<<\n', t_idx);
    ...
    for idx = 1:length(k_list)
        ...
        % A. Reconstruir Estado (Esto reinicia 'estado' a las 16 variables base)
        [estado, params] = reconstruct_state_matlab_3mg(k_target, TIPO_MODELO);
        
        % --- FEATURE ENGINEERING EXTENDIDO (34 Variables) ---
        ...
        % 3. Desviaciones (9 vars)
        std_P_gen = std(P_gen, 0, 1);
        std_P_dem = std(P_dem, 0, 1);
        std_Q_dem = std(Q_dem, 0, 1);
        
        % Construir Vector Base (Posiciones 1-16)
        x_base = estado.X_original;
        base_idx = [3, 8, 13];
        for m = 1:3
            bi = base_idx(m);
            x_base(bi) = m_P_dem(m); x_base(bi+1) = m_P_gen(m); x_base(bi+2) = m_Q_dem(m);
            
            % Asegurar prefijo 'Mean_' para claridad
            for p=0:2
                curr = estado.feature_names{bi+p};
                if ~startsWith(curr, 'Mean_'), estado.feature_names{bi+p} = ['Mean_' curr]; end
            end
        end
        
        % Concatenar Vector Completo (34 Vars)
        ...
        % Actualizar nombres en el estado actual
        estado.feature_names = [estado.feature_names, new_names];
        
        % Guardar estructura de nombres (solo necesitamos la del primer paso como referencia)
        if idx == 1
            temporal_results.feature_names = estado.feature_names;
        end
        
        % B. Valor Real y Compilador
        ...
        % C. Ejecutar Wrapper DENOISING
        % Nota: Este wrapper debe cargar el modelo AE de 34 variables y usar el predict_fn MOMENTS
        [lime_stats, all_explanations] = calculate_lime_pumping_3mg_with_AE_MOMENTS(estado, controller_obj, params, NUM_RUNS_PER_POINT, t_idx);
        ...
        fprintf('  K=%d: Q_p=%.3f, R2=%.4f\n', k_target, val_real, lime_stats.R2_mean);
    end
    
    filename_out = sprintf('lime_temporal_PUMP_%s_MG%d_7days_MOMENTS_DENOISING.mat', TIPO_MODELO, t_idx);
    save(filename_out, 'temporal_results');
    fprintf('  [Guardado]: %s\n', filename_out);
end
fprintf('\n--- FIN PROCESO MOMENTS DENOISING ---\n');

% --- Helper Compilador (Sin cambios) ---
