%% --- File: plot_temporal_pumping_evolution.m ---
%
% MASTER VISUALIZATION SCRIPT FOR TEMPORAL LIME (Pumping Q_p)
%
% FOCUS: NEIGHBOR INTERACTION & AGENT INFLUENCE
%
% Generates 3 types of plots per agent:
% 1. AREA PLOT: Aggregated influence by AGENT (MG1, MG2, MG3, Aquifer).
%    (Matches the style of the Q_t analysis).
% 2. CORRELATION PLOT: Physical Pumping vs. Neighbor Influence.
% 3. WEIGHTS EVOLUTION: Line plot of the Top 8 most influential features.
%
% Updates:
% - Changed aggregation logic from 'Physical Drivers' to 'Agents'.
% - Consistent styling (Times New Roman, Thesis Notation).
% - Dual Export (Paper/Presentation).
% - Time Shift (Days 1-7).
%
%--------------------------------------------------------------------------
clear; clc; close all;
...
% Colors for Area Plot (Agent-Based)
...
fprintf('--- GENERATING TEMPORAL PUMPING PLOTS (Interaction Focus) ---\n');

for t_idx = TARGETS
    
    % Load Data
    filename = sprintf('lime_temporal_PUMP_MG%d_7days.mat', t_idx);
    ...
    % --- 1. AGGREGATE INFLUENCE BY AGENT (MG1, MG2, MG3, Aquifer) ---
    ...
    % Generate Latex Labels (Thesis Notation)
    ...
    % B. CORRELATION PLOT (Pumping vs Neighbor Influence)
...
fprintf('--- ALL TEMPORAL PUMPING PLOTS EXPORTED ---\n');
