%% --- Archivo: calculate_lime_pumping_3mg_STANDARD_MOMENTS_PARETO.m ---
%
% LIME STANDARD (Sin AE) para variable de BOMBEO (Q_p).
% Utiliza 34 variables (Base + Momentos).
% Perturbación: PARETO (Cola pesada, según paper original).
%--------------------------------------------------------------------------
function [lime_stats, all_explanations] = calculate_lime_pumping_3mg_STANDARD_MOMENTS_PARETO(estado, controller_obj, params, NUM_EXECUTIONS, TARGET_MG_IDX)
    ...
    % --- 2. PARÁMETROS LIME (PARETO) ---
    alpha = 3.0;      % Forma de la distribución (Paper)
    strength = 0.20;  % Intensidad (20%)
    num_synthetic = 1000; 
    kernel_width = 0.75 * sqrt(N_features); % Ancho ajustado para Pareto
    ...
    for run_idx = 1:NUM_EXECUTIONS
        ...
        % A. Generación de Perturbaciones (PARETO EXACTA)
        % 1. Magnitud Pareto: rand^(-1/alpha)
        ...
        % 4. Aplicación Multiplicativa: X_pert = X + Strength * (X * Ruido)
