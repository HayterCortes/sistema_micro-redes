%% --- Archivo: lime_temporal_pumping_main_3mg_MOMENTS_DENOISING_RAW_RBO.m ---
%
% ANÁLISIS TEMPORAL LIME BOMBEO (Q_p) - AE MOMENTS (34 vars)
%
% Objetivo: Explicar Q_p usando proyección AE para garantizar realismo.
% Funcionalidades: Selector Perturbación, RBO, RAW Data.
%--------------------------------------------------------------------------
close all; clear; clc;
...
PERTURBATION_TYPE = 'PARETO'; % 'GAUSSIAN' o 'PARETO'

fprintf('--- LIME TEMPORAL PUMPING (AE MOMENTS) - Modelo: %s - Perturb: %s ---\n', TIPO_MODELO, PERTURBATION_TYPE);
...
% --- 4. BUCLE PRINCIPAL ---
for t_idx = TARGETS
    fprintf('\n>>> TEMPORAL PUMPING AE MOMENTS (%s) - MG %d <<<\n', PERTURBATION_TYPE, t_idx);
    ...
    for idx = 1:length(k_list)
        ...
        if strcmp(PERTURBATION_TYPE, 'PARETO')
            [lime_stats, all_explanations] = calculate_lime_pumping_3mg_with_AE_MOMENTS_PARETO(estado, controller_obj, params, NUM_RUNS_PER_POINT, t_idx);
        else
    ...
    filename_out = sprintf('lime_temporal_pumping_%s_MG%d_7days_AE_MOMENTS_RAW_RBO_%s.mat', TIPO_MODELO, t_idx, PERTURBATION_TYPE);
