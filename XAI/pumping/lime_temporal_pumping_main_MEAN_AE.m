%% --- Archivo: lime_temporal_pumping_main_MEAN_AE.m ---
%
% ANÁLISIS TEMPORAL LIME PARA BOMBEO (Q_p) - VERSIÓN MEAN + AR/TS + AE (Autoencoder)
% Ejecuta LIME usando proyección en Manifold (Autoencoder) para mejorar R2.
%
% SALIDA: Archivos con sufijo '_AE.mat' para no sobrescribir los originales.
%--------------------------------------------------------------------------
close all; clear; clc;

% --- 1. CONFIGURACIÓN ---
TIPO_MODELO = 'TS';      % <--- CAMBIA ESTO A 'AR' O 'TS' (Recuerda correr ambos)
...
fprintf('--- LIME TEMPORAL BOMBEO (MEAN + AE Denoising) - Modelo: %s ---\n', TIPO_MODELO);
...
for t_idx = TARGETS
    fprintf('\n==================================================\n');
    fprintf('   EVOLUCIÓN TEMPORAL BOMBEO (AE): AGENTE MG %d\n', t_idx);
    fprintf('==================================================\n');
    ...
    for idx = 1:length(k_list)
        ...
        % D. Ejecutar LIME con AUTOENCODER
        [lime_stats, all_explanations] = calculate_lime_pumping_3mg_with_AE(estado, controller_obj, params, NUM_RUNS_PER_POINT, t_idx);
    ...
    % Guardado CON DISTINTIVO _AE
    filename_out = sprintf('lime_temporal_PUMP_%s_MG%d_7days_MEAN_AE.mat', TIPO_MODELO, t_idx);
    save(filename_out, 'temporal_results');
    fprintf('--> Datos guardados en %s\n', filename_out);
end
fprintf('\n--- ANÁLISIS TEMPORAL DE BOMBEO (AE) FINALIZADO ---\n');
