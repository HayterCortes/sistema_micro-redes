%% --- Archivo: calculate_lime_pumping_3mg.m ---
%
% Ejecuta LIME para explicar Q_p (Bombeo).
% Diferencia clave: Usa predict_fn_mpc_wrapper_pumping.
%--------------------------------------------------------------------------
function [all_explanations, feature_names] = calculate_lime_pumping_3mg(estado, controller_obj, params, NUM_EXECUTIONS, TARGET_MG_IDX)

    X_original = estado.X_original; 
    feature_names = estado.feature_names;
    N_features = length(X_original);
    
    % Construir Límites (Igual que antes)
    ...
    fprintf('Iniciando LIME BOMBEO (Target: Q_p MG%d)...\n', TARGET_MG_IDX);
    ...
    for run_idx = 1:NUM_EXECUTIONS
        ...
        % C. Y-Scaler (USANDO WRAPPER DE BOMBEO)
