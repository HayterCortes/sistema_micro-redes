%% --- Archivo: lime_temporal_pumping_main_3mg_STANDARD_RAW_RBO.m ---
%
% ANÁLISIS TEMPORAL LIME BOMBEO (Q_p) - STANDARD 34 VARS (Sin AE)
%
% Objetivo: Explicar Q_p usando 34 variables (Base + Max + Std) sin proyección AE.
% Funcionalidades: Selector de Perturbación, RBO (Estabilidad) y RAW Data.
%--------------------------------------------------------------------------
close all; clear; clc;
...
try
    ...
    if isempty(fname)
        error('Datos no encontrados. Verifique la carpeta results_mpc.');
    end
...
for t_idx = TARGETS
    fprintf('\n==================================================\n');
    fprintf('   INICIANDO EVOLUCIÓN PUMPING (STD): MG %d\n', t_idx);
    fprintf('==================================================\n');
    ...
    for idx = 1:length(k_list)
        ...
        % Nombres
        for i=1:length(estado.feature_names)
            if ~startsWith(estado.feature_names{i},'Mean_') && i~=1 && i~=2 && i~=6 && i~=7 && i~=11 && i~=12 && i~=16
                estado.feature_names{i} = ['Mean_' estado.feature_names{i}]; 
            end
        end
        ...
        fprintf('K=%d (Q_p=%.3f)... ', k_target, val_real);
        
        % *** E. SELECCIÓN DE MÉTODO LIME ***
        if strcmp(PERTURBATION_TYPE, 'PARETO')
            [lime_stats, all_explanations] = calculate_lime_pumping_3mg_STANDARD_MOMENTS_PARETO(...
                estado, controller_obj, params, NUM_RUNS_PER_POINT, t_idx);
        else
            [lime_stats, all_explanations] = calculate_lime_pumping_3mg_STANDARD_MOMENTS(...
                estado, controller_obj, params, NUM_RUNS_PER_POINT, t_idx);
        end
        
        temporal_results.quality_history(idx) = lime_stats.R2_mean;
        
        % --- F. CÁLCULO DE RBO TEMPORAL ---
    ...
    filename_out = sprintf('lime_temporal_pumping_%s_MG%d_7days_STANDARD_RAW_RBO_%s.mat', ...
        TIPO_MODELO, t_idx, PERTURBATION_TYPE);
    
    save(filename_out, 'temporal_results');
    fprintf('--> Datos guardados en %s\n', filename_out);
end
fprintf('\n--- FIN STANDARD RAW RBO (PUMPING) ---\n');

%% --- Helper: Cálculo de RBO ---
