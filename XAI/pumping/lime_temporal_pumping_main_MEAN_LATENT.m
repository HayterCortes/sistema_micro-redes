%% --- Archivo: lime_temporal_pumping_main_MEAN_LATENT.m ---
%
% ANÁLISIS TEMPORAL BOMBEO (Q_p) - VERSIÓN LATENT SPACE SAMPLING
%
% Ejecuta LIME perturbando el vector latente (Z) del Autoencoder.
% SALIDA: Archivos con sufijo '_LATENT.mat'.
%--------------------------------------------------------------------------
close all; clear; clc;

% --- 1. CONFIGURACIÓN ---
TIPO_MODELO = 'TS';      % <--- CAMBIA A 'AR' y LUEGO A 'TS'
TARGETS = [1, 2, 3];     
INTERVALO_HORAS = 12;    
NUM_RUNS_PER_POINT = 1;  

fprintf('--- LIME BOMBEO (LATENT SPACE) - Modelo: %s ---\n', TIPO_MODELO);
...
try
    ...
    if isempty(fname)
         error('No se encuentran los datos de resultados MPC.');
    end
    results = load(fname);
    mg = results.mg;
...
for t_idx = TARGETS
    ...
    temporal_results.target_real_history = []; % Inicializar historial real
    ...
    for idx = 1:length(k_list)
        ...
        m_P_dem = mean(estado.constants.p_dem_pred_full, 1); 
        m_P_gen = mean(estado.constants.p_gen_pred_full, 1); 
        m_Q_dem = mean(estado.constants.q_dem_pred_full, 1);
        ...
        % C. Obtener Valor Real (CORREGIDO: Faltaba esta sección)
        val_real = results.Q_p(k_target, t_idx);
        temporal_results.target_real_history(idx) = val_real;

        % D. Compilar y Ejecutar Wrapper LATENT
        controller_obj = get_compiled_mpc_controller_3mg(params.mg);
        
        [lime_stats, all_explanations] = calculate_lime_pumping_3mg_LATENT(estado, controller_obj, params, NUM_RUNS_PER_POINT, t_idx);
        ...
        % PRINT ACTUALIZADO: Ahora muestra Q_p
        fprintf('  K=%d: Q_p=%.3f, R2=%.4f\n', k_target, val_real, lime_stats.R2_mean);
    end
    
    % Guardado
    filename_out = sprintf('lime_temporal_PUMP_%s_MG%d_7days_MEAN_LATENT.mat', TIPO_MODELO, t_idx);
...
function Controller = get_compiled_mpc_controller_3mg(mg_array)
    % (Tu código de compilador YALMIP estándar va aquí, idéntico a los otros scripts)
    yalmip('clear');
