%% --- File: plot_quality_comparison_AR_TS_all_AE.m ---
%
% COMPARA LA CALIDAD (R2) LIME CON AUTOENCODER: AR vs TS
%
% FUNCIONALIDADES:
% 1. Carga los resultados "MEAN_AE" (con denoising neuronal).
...
% --- CONFIGURACIÓN ---
SCENARIO_TYPE = 'PUMP';  % <--- Normalmente AE lo aplicamos a PUMP
TARGETS = [1, 2, 3];     % Microrredes a analizar
LIMITE_CALIDAD = 0.5;    % Umbral visual

% Directorio de salida (Diferenciado para no mezclar)
out_dir = 'figures_comparison_quality_AE';
if ~exist(out_dir, 'dir'), mkdir(out_dir); end

fprintf('==========================================================\n');
fprintf('   ANÁLISIS COMPARATIVO LIME + AUTOENCODER (AR vs TS)\n');
fprintf('   Escenario: %s\n', SCENARIO_TYPE);
fprintf('==========================================================\n\n');

% Definir patrones de archivo (ACTUALIZADO PARA _AE)
if strcmp(SCENARIO_TYPE, 'PUMP')
    % Aquí buscamos el archivo con sufijo _AE
    file_pattern = 'lime_temporal_PUMP_%s_MG%d_7days_MEAN_AE.mat';
    title_var = 'Bombeo (Q_p) + AE Denoising';
else
    % Si aplicaste AE a STD, cambia esto. Si no, usa el normal.
...
for t_idx = TARGETS
    ...
    fprintf('>>> PROCESANDO MICRORRED %d (AE) <<<\n', t_idx);
    ...
    if ~isfile(fname_AR) || ~isfile(fname_TS)
        fprintf('  [!] Faltan archivos _AE para MG%d. ¿Corriste el script actualizado?\n', t_idx);
        fprintf('      Buscando: %s\n', fname_AR);
        continue;
    end
    ...
    fprintf('  Tabla Comparativa de Fidelidad con AE (R^2) - MG%d:\n', t_idx);
    disp(T);
    fprintf('\n');
    
    % 3. GENERAR GRÁFICO COMPARATIVO
    fig = figure('Name', sprintf('Quality_AE_MG%d', t_idx), 'Position', [100, 100, 900, 500], 'Color', 'w', 'Visible', 'off');
    ...
    linea2 = sprintf('Agente: Microgrid %d (Mejorado con Autoencoder)', t_idx);
    ...
    % 4. EXPORTAR (SOLO PDF)
    out_name = sprintf('Comparison_Quality_R2_AE_%s_MG%d', SCENARIO_TYPE, t_idx);
