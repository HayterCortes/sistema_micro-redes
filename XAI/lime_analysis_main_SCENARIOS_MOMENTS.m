%% --- Archivo: lime_analysis_main_SCENARIOS_MOMENTS_RBO.m ---
%
...
%
% MODIFICACIÓN: Incluye cálculo de Rank Biased Overlap (RBO) para medir estabilidad.
%--------------------------------------------------------------------------
close all; clear; clc;
...
fprintf('--- LIME INTERCAMBIO (SCENARIOS + MOMENTS + RBO) - Modelo: %s - Runs: %d ---\n', TIPO_MODELO, NUM_RUNS);
...
for s_idx = 1:length(scenarios)
    ...
    for t_idx = 1:3
        ...
        % --- D. CÁLCULO DE RBO (Rank Biased Overlap) ---
        ...
        % Guardar
        filename = sprintf('lime_Scenario_%s_%s_MG%d_MOMENTS_RBO.mat', scn.name, TIPO_MODELO, t_idx);
...
fprintf('\n=== FIN ANÁLISIS ESCENARIOS MOMENTS CON RBO ===\n');
